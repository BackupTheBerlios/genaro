...
toSeq xs = tos xs S.empty
  where tos E rest = rest
        tos (T a x b) rest = S.cons x (tos a (tos b rest))
...
lookupAll xs x = look xs x S.empty
  where look E x rest = rest
        look (T a y b) x rest
          | x < y     = look a x rest
          | x > y     = look b x rest
          | otherwise = look a x (S.cons y (look b x rest))
...
toOrdSeq xs = tos xs S.empty
  where tos E rest = rest
        tos (T a x b) rest = tos a (S.cons x (tos b rest))
