...
construyeListaLineasWalking (aleat@(a1:restoAleat1), (listaMutsDursTonicas@((ton1, durTotal, numAlarga, numInter, numTrina, escala):(ton2,d,nA,nI,nT,e):ls), durNotas, despCurva, numDivisiones, octavaPartida, primerPitch))= (restoAleat3, musAux:restoMus)
    where p1 = (ton1, octavaPartida)
          ---(restoAleat1, oct2) = dameOctavaIniAleat registroDelBajo aleat
          (despOct,_) = dameElemAleatListaPesos a1 [(-1,2), (0, 1), (1,2)]
          oct2 = ajustaOctava registroDelBajo (octavaPartida + despOct)
          p2 = (ton2, oct2)
